...
// dia_semana: 0=domingo .. 6=sábado. Exibição começa na segunda.
const DIAS = [
  { n: 1, label: "Segunda-feira" }, { n: 2, label: "Terça-feira" }, { n: 3, label: "Quarta-feira" },
  { n: 4, label: "Quinta-feira" }, { n: 5, label: "Sexta-feira" }, { n: 6, label: "Sábado" }, { n: 0, label: "Domingo" },
];
    ...
    <div className="p-6">
      <header className="mb-5 flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-black/50">Cadastre a equipe, suas escalas e ausências. O agente agenda na agenda certa.</p>
        </div>
      ...
      {profs.length === 0 ? (
        <p className="rounded-xl border border-dashed border-black/15 p-10 text-center text-sm text-black/40">
          Nenhum profissional ainda. Cadastre o primeiro para montar a agenda.
        </p>
      ) : (
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <div className="text-xs text-black/45">
                    {p.escalas.length} dia(s) de escala{!p.ativo && " · inativo"}
                  </div>
                </div>
              </div>
...
function ModalProfissional({ prof, servicos, onClose, onSaved }: {
  ...
  const [dias, setDias] = useState<Record<number, DiaEscala>>(() => {
    ...
    for (const d of DIAS) m[d.n] = { ...DIA_PADRAO };
  ...
  function escalasPayload(): Escala[] {
    return DIAS.filter((d) => dias[d.n].ativo).map((d) => {
      const e = dias[d.n];
      return {
        dia_semana: d.n, hora_inicio: e.hora_inicio, hora_fim: e.hora_fim, intervalo_min: e.intervalo_min,
    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 p-4" onClick={onClose}>
      <div className="flex max-h-[90vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl bg-white shadow-xl" onClick={(e) => e.stopPropagation()}>
        <div className="border-b border-black/10 px-5 py-4">
          <h2 className="text-lg font-semibold">{prof ? `Editar — ${prof.nome}` : "Novo profissional"}</h2>
        </div>
        <div className="flex gap-1 border-b border-black/10 px-3">
          {(["dados", "escala", "ausencia"] as const).map((a) => (
            <button key={a} onClick={() => setAba(a)}
              className={`-mb-px border-b-2 px-3.5 py-2.5 text-sm font-medium capitalize transition ${
                aba === a ? "border-brand text-brand" : "border-transparent text-black/50 hover:text-ink"
              }`}>
              {a === "ausencia" ? "Ausência" : a}
            </button>
          ))}
        </div>
        ...
        <div className="flex-1 overflow-auto p-5">
          {aba === "dados" && (
            <div>
              ...
              <input value={nome} onChange={(e) => setNome(e.target.value)} placeholder="Ex.: Dr. Arthur"
              ...
              <label className="mb-2 block text-xs font-medium text-black/55">Serviços habilitados</label>
              {servicos.length === 0 ? (
                <p className="rounded-lg bg-amber-50 p-2.5 text-xs text-amber-800">Cadastre serviços primeiro (menu Serviços) para habilitar aqui.</p>
              ) : (
              ...
              <label className="mt-4 flex items-center gap-2 text-sm">
                <input type="checkbox" checked={ativo} onChange={(e) => setAtivo(e.target.checked)} /> Profissional ativo
              </label>
            </div>
          )}
          ...
          {aba === "escala" && (
            <div className="space-y-2">
              {DIAS.map((d) => {
                const e = dias[d.n];
                return (
                  <div key={d.n} className={`rounded-xl border p-3 ${e.ativo ? "border-black/15 bg-white" : "border-black/10 bg-black/[0.02]"}`}>
                    <label className="flex items-center gap-2 text-sm font-medium">
                      <input type="checkbox" checked={e.ativo} onChange={(ev) => setDia(d.n, { ativo: ev.target.checked })} />
                      {d.label}
                    </label>
                    {e.ativo && (
                      <div className="mt-2.5 grid grid-cols-2 gap-2.5 sm:grid-cols-4">
                        <Campo label="Início"><input type="time" value={e.hora_inicio} onChange={(ev) => setDia(d.n, { hora_inicio: ev.target.value })} className={inputCls} /></Campo>
                        <Campo label="Fim"><input type="time" value={e.hora_fim} onChange={(ev) => setDia(d.n, { hora_fim: ev.target.value })} className={inputCls} /></Campo>
                        <Campo label="Intervalo">
                          <select value={e.intervalo_min} onChange={(ev) => setDia(d.n, { intervalo_min: Number(ev.target.value) })} className={inputCls}>
                        ...
                        <Campo label="Almoço">
                          <label className="flex h-[34px] items-center gap-1.5 text-xs text-black/60">
                            <input type="checkbox" checked={e.almoco} onChange={(ev) => setDia(d.n, { almoco: ev.target.checked })} /> tem
                          </label>
                        </Campo>
                        {e.almoco && (
                          <>
                            <Campo label="Almoço início"><input type="time" value={e.almoco_inicio} onChange={(ev) => setDia(d.n, { almoco_inicio: ev.target.value })} className={inputCls} /></Campo>
                            <Campo label="Almoço fim"><input type="time" value={e.almoco_fim} onChange={(ev) => setDia(d.n, { almoco_fim: ev.target.value })} className={inputCls} /></Campo>
                          </>
                        )}
                      </div>
                    )}
          ...
          {aba === "ausencia" && (
            prof ? <AbaAusencia profId={prof.id} />
              : <p className="rounded-lg bg-amber-50 p-3 text-sm text-amber-800">Salve o profissional primeiro para cadastrar ausências.</p>
          )}
        </div>
        ...
        <div className="flex justify-end gap-2 border-t border-black/10 px-5 py-3">
          ...
          <button onClick={salvar} disabled={salvando || !nome.trim()}
            ...
            {salvando ? "Salvando…" : prof ? "Atualizar" : "Criar"}
          </button>
        </div>
      </div>
    </div>
    ...
    <div>
      <div className="rounded-xl border border-black/10 bg-paper p-3.5">
        <div className="mb-2 flex gap-2">
          {(["dia_todo", "horas"] as const).map((t) => (
            <button key={t} type="button" onClick={() => setTipo(t)}
              className={`rounded-lg px-3 py-1.5 text-xs font-medium transition ${tipo === t ? "bg-brand text-white" : "border border-black/15 text-black/60"}`}>
              {t === "dia_todo" ? "Dia inteiro" : "Por horas"}
            </button>
          ))}
        </div>
        <div className="grid grid-cols-2 gap-2.5">
          <Campo label="De"><input type="date" value={dataInicio} onChange={(e) => setDataInicio(e.target.value)} className={inputCls} /></Campo>
          <Campo label="Até"><input type="date" value={dataFim} onChange={(e) => setDataFim(e.target.value)} className={inputCls} /></Campo>
          {tipo === "horas" && (
            <>
              <Campo label="Hora início"><input type="time" value={horaInicio} onChange={(e) => setHoraInicio(e.target.value)} className={inputCls} /></Campo>
              <Campo label="Hora fim"><input type="time" value={horaFim} onChange={(e) => setHoraFim(e.target.value)} className={inputCls} /></Campo>
            </>
          )}
        </div>
      ...
      <div className="mt-3 space-y-1.5">
        {lista.length === 0 && <p className="py-3 text-center text-xs text-black/35">Nenhuma ausência cadastrada.</p>}
        {lista.map((a) => (
          <div key={a.id} className="flex items-center gap-2 rounded-lg border border-black/10 bg-white px-3 py-2 text-sm">
            ...
            <button onClick={() => api.apagarAusencia(profId, a.id).then(carregar)} aria-label="Apagar"
